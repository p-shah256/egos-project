...
m_uint16 macb_mdio_read(m_uint8 phy_adr, m_uint8 reg)
{
	...
	frame = (MACB_BF(SOF, 1) | MACB_BF(RW, 2) | MACB_BF(PHYA, phy_adr) | MACB_BF(REGA, reg) | MACB_BF(CODE, 2));
...
void macb_start()
{
	...
	for (i = 0; i < MACB_TX_RING_SIZE; i++) {
		...
		if (i == (MACB_TX_RING_SIZE - 1))
			macb.tx_ring[i].ctrl = MACB_BIT(TX_USED) | MACB_BIT(TX_WRAP);
		else
...
int _macb_write_hwaddr()
{
	...
	hwaddr_bottom = enetaddr[0] | enetaddr[1] << 8 | enetaddr[2] << 16 | enetaddr[3] << 24;
